...
export default function MonthlyReportPage() {
  ...
  return <DashboardShell><div className="space-y-6">
    ...
    {isAdmin ? <ReportHistory reports={reports} onDownload={download} onDelete={deleteReport} /> : user?.role !== "hunter" ? <div className="card">MONTHLY REPORT hanya tersedia untuk Sales Leader / Sales Manager.</div> : <>
      <section className="rounded-xl border p-5 space-y-4" style={{ background: "var(--surface)", borderColor: "var(--border)" }}><div className="grid md:grid-cols-4 gap-3"><Field label={getHunterTitle(user.name)} value={user.name} disabled /><Field label="Bulan Laporan" value={reportMonth} type="month" onChange={changeReportMonth} /><Field label="Periode Otomatis" value={`${periodStart} – ${periodEnd}`} disabled /><Field label="Coverage" value={profile.project_coverage.join(", ") || "Belum diatur"} disabled /></div></section>
