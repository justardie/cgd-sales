...
export default function ReportPage() {
  ...
  const loadReports = useCallback(async () => {
    if (!user) return
    let query = supabase.from("weekly_reports").select("id,hunter_name,period_start,period_end,status,snapshot,updated_at").order("updated_at", { ascending: false })
  ...
  async function save(status: "draft"|"final") {
    ...
    setBusy(true)
    const payload = { user_id: user.id, hunter_name: user.name, period_start: periodStart, period_end: periodEnd, status, activities: snapshot.activities, visit_data: visits, pivot_filename: pivotFilename || null, snapshot: status === "final" ? snapshot : null, finalized_at: status === "final" ? new Date().toISOString() : null, updated_at: new Date().toISOString() }
    ...
    setMessage(status === "final" ? "Laporan difinalisasi dan diunduh." : "Draft tersimpan.")
    if (status === "final") download(snapshot)
    await loadReports()
  }
  ...
  return <DashboardShell><div className="space-y-6">
    ...
    {isAdmin ? <ReportHistory reports={reports} onDownload={download} /> : user?.role !== "hunter" ? <div className="card">REPORT hanya tersedia untuk Sales Hunter.</div> : <>
      ...
      {message&&<p className="text-sm text-amber-300">{message}</p>}<div className="flex gap-3"><button disabled={busy} onClick={()=>void save("draft")} className="btn-secondary flex items-center gap-2"><Save size={16}/> Simpan Draft</button><button disabled={busy} onClick={()=>void save("final")} className="btn-primary flex items-center gap-2"><Download size={16}/> Finalisasi &amp; Download</button></div>
      <ReportHistory reports={reports} onDownload={download}/>
    </>}
  </div></DashboardShell>
}

function Field({label,value,onChange,type="text",disabled=false}:{label:string;value:string;onChange?:(v:string)=>void;type?:string;disabled?:boolean}) { return <label className="text-xs text-slate-500">{label}<input className="input-dark mt-1 w-full" type={type} disabled={disabled} value={value} onChange={e=>onChange?.(e.target.value)}/></label> }
function ReportHistory({reports,onDownload}:{reports:StoredReport[];onDownload:(s:ReportSnapshot)=>void}) { return <section className="rounded-xl border p-5" style={{background:"var(--surface)",borderColor:"var(--border)"}}><h2 className="font-semibold text-white mb-3">Riwayat Report</h2>{reports.length===0?<p className="text-sm text-slate-500">Belum ada report.</p>:<div className="space-y-2">{reports.map(r=><div key={r.id} className="flex items-center justify-between border-b py-2" style={{borderColor:"var(--border)"}}><div className="text-sm text-slate-300"><b>{r.hunter_name}</b> · {r.period_start} – {r.period_end} <span className={r.status==="final"?"text-emerald-400":"text-amber-400"}>{r.status}</span></div>{r.snapshot&&<button className="text-emerald-400" onClick={()=>onDownload(r.snapshot!)}><Download size={17}/></button>}</div>)}</div>}</section> }
