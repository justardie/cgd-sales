...
test("Overview keeps project totals without the donut and aligns KPI cards", async () => {
  ...
  assert.match(source, /grid-cols-2 lg:grid-cols-5/)
  assert.match(source, /w-12 h-12 sm:w-\[76px\] sm:h-\[76px\]/)
  assert.match(source, /text-sm sm:text-xl font-bold leading-none/)
  assert.match(source, /label="Sales Aktif"/)
  assert.match(source, /KPI_LABEL_CLASS/)
  assert.match(source, /KPI_VALUE_CLASS/)
  assert.match(source, /KPI_SUB_CLASS/)
  assert.doesNotMatch(source, /label="Sales Person Aktif"/)
  assert.doesNotMatch(source, /icon=\{(?:TrendingUp|DollarSign|Activity|Users)\}/)
...
test("Premium Dashboard motion is native, accessible, and dependency-free", async () => {
  const pkg = await read("package.json")
  const css = await read("app/globals.css")
  const page = await read("app/page.tsx")
  const motion = await read("components/DashboardMotion.tsx")
  const shell = await read("components/DashboardShell.tsx")
  const notification = await read("components/NotificationBell.tsx")
  const modal = await read("components/Modal.tsx")
  const toast = await read("contexts/ToastContext.tsx")
