import { getHunterTitle } from "./hunters.ts"
 ...
 * summed per person; otherwise only the single matching month is used. By
 * default, falls back to the most recent group if no target month is present.
 */
export function parsePivotSheet(raw: (string | number)[][], targets?: { year: number; month: number }[], strictTarget = false): PivotVisitRow[] {
  ...
  const matchingGroups = targets
    ...
    : []
  if (strictTarget && targets?.length && matchingGroups.length === 0) throw new Error("Data Pivot untuk bulan laporan tidak ditemukan.")
...
export function buildReportHtml(data: ReportSnapshot, reportType: "weekly" | "monthly" = "weekly") {
  const hunterTitle = getHunterTitle(data.hunterName)
  const visitTarget = reportType === "monthly" ? data.visitTarget ?? 40 : 40
  ...
  const reportTitle = reportType === "monthly" ? "SALES MONTHLY REPORT" : "SALES WEEKLY REPORT"
  const closingCopy = reportType === "monthly" ? "Closing Bulanan" : "Closing MTD"
  const omsetCopy = reportType === "monthly" ? "Omset Bulanan" : "Omset MTD"
  const closingThroughCopy = reportType === "monthly" ? "Closing Bulanan sampai" : "Closing MTD sampai"
  const totalOmsetCopy = reportType === "monthly" ? "TOTAL OMSET BULANAN" : "TOTAL OMSET MTD"
  const finalSection = reportType === "monthly"
    ? `<section class="section"><div class="sec-head"><span class="sec-num">4</span><span class="sec-title">Monthly Review</span></div><div class="sec-body"><div class="visit-grid"><div class="vcard"><b>What's Good</b><div style="white-space:pre-wrap">${esc(data.monthlyReview?.good)}</div></div><div class="vcard"><b>What's Bad</b><div style="white-space:pre-wrap">${esc(data.monthlyReview?.bad)}</div></div><div class="vcard"><b>What's Next</b><div style="white-space:pre-wrap">${esc(data.monthlyReview?.next)}</div></div></div></div></section>`
  ...
  return `<!doctype html><html lang="id"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width"><title>${reportTitle} - ${esc(data.hunterName)}</title><style>
